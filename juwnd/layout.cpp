...
namespace ju{
	...
	void LayoutBinder::_onSize(Message* msg,IWnd*){
		if(msg->wParam==SIZE_MINIMIZED) return;
		Twin16 cs = *((Twin16*)&msg->lParam);
		// Scroll 版本的
		int w = Maximum(_container->MinSize().x,(int)cs.x);
	...
	void LayoutBinder::Reset(){
		Lay.Reset();
		Lay.Name = L"Root";
		Lay.Param.Weight = 0;
		Lay.Param.Margin.SetValue(10,10,10,10);
		if(_container!=NULL){
			_container->GetMsgHook(WM_SIZE)->Delete(this,&LayoutBinder::_onSize);
		}
		_container = NULL;
	}
	LayoutBinder::LayoutBinder():_container(0),_containerIsScrollView(0){
		Lay.Name = L"Root";
		Lay.Param.Margin.SetValue(10,10,10,10);
		Lay.Param.Weight = 0;
	}
	void LayoutBinder::Refresh(HWND parent){
		Twin16 cs;
		if(!_container->GetClientSize(cs)) return;
		int w = Maximum(_container->MinSize().x,(int)cs.x);
		int h = Maximum(_container->MinSize().y,(int)cs.y);
	...
	void LayoutBinder::Bind(IWnd* wnd,bool isScrollView){
		...
		_container = wnd;
		if(!wnd) return;
		wnd->GetMsgHook(WM_SIZE)->Add(this,&LayoutBinder::_onSize);
		Refresh();
	}
