...
namespace ju{
	...
	bool ListViewPtr::SetItemData(int index, void* data){
		LVITEM lvi;
		::ZeroMemory(&lvi,sizeof(lvi));
		lvi.mask = LVIF_PARAM;
	...
	void* ListViewPtr::GetItemData(int index){
		LVITEM lvi;
		::ZeroMemory(&lvi,sizeof(lvi));
		lvi.mask = LVIF_PARAM;
		lvi.iItem = index;
		this->SendMessage(LVM_GETITEM,0,&lvi);
	...
	LRESULT ListViewPtr::SetItemImage(int iItem,int iImage){
		LVITEM lvi;
		::ZeroMemory(&lvi,sizeof(lvi));
		lvi.iImage = iImage;
		lvi.iSubItem = 0;
