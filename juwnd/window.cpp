...
namespace ju{
	...
	Font* GetDefaultFont(){
		static Font* _font = 0;
		if(_font==0){
			MutexLock ml(L"{61771393-873D-4d52-AC49-8CD9A6DFAE3F}");
			if(_font==0){
				_font = NewStd<Font>();
				if(_font->IsNull()){
					_font->Create();
	...
	IWnd::IWnd(void):FontColor(0),_Scr(0,0),_MinSize(0,0),_mLeave(true),_mNcLeave(true),
		...
		Param.New();
		DragMinSize.SetValue(0,0);
		DragMaxSize.SetValue(0xffff,0xffff);
	...
	LRESULT IWnd::_WndProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam){
		...
		if((uMsg>=WM_MOUSEFIRST&&uMsg<=WM_MOUSELAST)){//处理鼠标消息.
			...
			ICtrl* self = GetICtrl(hWnd);
			if(self&&!self->OnNotify.IsNull())
				self->OnNotify(&msg,wnd);
...
return_code:
	...
	void View::_DoubleDraw(DC* hdc,IWnd*){
		...
		::BitBlt(hdc->Handle(),clip.left,clip.top,clip.right,clip.bottom,dc,clip.left,clip.top,SRCCOPY);
		...
		::AlphaBlend(*hdc,0,0,rect.right,rect.bottom,dc,0,0,rect.right,rect.bottom,bf);*/
		bmp.Select(dc);
	}
