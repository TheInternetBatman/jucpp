...
#define WM_GET_IWND			(WM_CTRLUSER+0x01869)		//从HWND获取Window*指针.
#define WM_GET_ICTRL		(WM_CTRLUSER+0x02867)		//从HWND获取Window*指针.
...
namespace ju{
	...
	//窗口函数的返回值类型.
	class JUWND_API Result : public HandleType<INT_PTR>{
	public:
	...
	//Windows 消息结构
	typedef struct JUWND_API Message : public _struct{
		HWND		hWnd;		//消息的窗口句柄.
		UINT		uMsg;		//消息ID
		WPARAM		wParam;		//参数
		LPARAM		lParam;		//参数
	...
	//鼠标结构,用于鼠标消息.
	typedef struct JUWND_API Mouse : public _struct{
		Mouse(){}
	...
	//非客户去鼠标结构
	typedef struct JUWND_API NcMouse : public _struct{
		UINT Event;		//消息ID
	...
	typedef struct JUWND_API CreateStruct : public _struct{
	...
	class JUWND_API WndPtr : public HandleType<HWND>{
	public:
		WndPtr(){}
	...
	public:
		...
		//设置拖放时的最小尺寸
		Twin		DragMinSize;		//允许拖拽时的最小尺寸。
		//设置拖放时的最大尺寸
		Twin		DragMaxSize;		//允许拖拽时的最小尺寸。
	};
	JUWND_API IWnd* GetIWnd(HWND hWnd);
	//Class View
	class JUWND_API View : public IWnd{//public WndPtr//,
	protected:
	...
	//Class ScrollView
	class JUWND_API ScrollView : public View{
	public:
		ScrollView(void);
		~ScrollView();
	...
	class JUWND_API Frame : public ScrollView{
	public:
		Frame();
		~Frame();
	...
	//设置主窗口的图标，含大图标和小图标。
	JUWND_API void SetWndIcon(HWND h,ResID id);
