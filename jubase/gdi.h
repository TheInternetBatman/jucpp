...
namespace ju{
	...
	//但是函数并不失败.
	template<typename T> class JUBASETL_API GdiObject : public HandleType<T>
	{
	public:
		GdiObject()
		{
	...
	//颜色结构.
	typedef union JUBASE_API Color{
	public:
		Color();
		Color(int value);
	...
	//区域.
	class JUBASE_API Rgn : public GdiObject<HRGN>{
	public:
		enum{
			And = 1,	//相当于交集.
			Or = 2,		//相当于并集.
	...
	class JUBASE_API Pen : public GdiObject<HPEN>
	{
	public:
		//生成一个PEN对象.
	...
	class JUBASE_API Brush : public GdiObject<HBRUSH>
	{
	public:
	...
	class JUBASE_API DC : public HandleType<HDC>{
		int _flag;
		enum{
			type_null = 0,
			type_bind = 1,
	...
	public:
		...
		void Close(){
			if(!_Handle) return;
			if(_flag==type_create){
				::DeleteDC(_Handle);
			}else if(0xffff0000&_flag){
				::ReleaseDC((HWND)(UINT_PTR)_flag,_Handle);
			}
			_flag = 0;
			_Handle = 0;
		}
	...
	class JUBASE_API Bitmap : public GdiObject<HBITMAP>{
	public:
		inline Bitmap(){}
		//用一个位图句柄初始化.
		...
		//把位图的指定矩形刷成指定的颜色，此函数不能把图像刷成透明（默认32位位图就是透明的），0 表示黑色。
		bool PaintColor(int color,int x = 0,int y = 0,int cx = 0,int cy = 0);
		//生成一个2位的位图.
		...
		bool Draw(HBITMAP bmp,int x = 0,int y = 0,int cx = 0,int cy = 0,int sx = 0,int sy = 0,int scx = 0,int scy = 0,DWORD raster = SRCCOPY);
		...
		bool DrawAlpha(HBITMAP bmp,BYTE alpha = 255,bool alphaChannel = 0,int x = 0,int y = 0,int cx = 0,int cy = 0,int sx = 0,int sy = 0,int scx = 0,int scy = 0);
	};
	//这个类型用于绘制位图,他的类型是一个HDC而非HBITMAP.
	/*class JUBASE_API DrawBitmap : public HandleType<HDC>
	{
	protected:
	...
	class JUBASE_API Icon : public HandleType<HICON>
	{
	public:
		Icon(void);
	...
	JUBASE_API bool IconToBlackWhite(Icon* icon);
	JUBASE_API bool BitmapToBlackWhite(Bitmap& bmp);
	JUBASE_API HBITMAP CreateBitmap(HDC hdc,int cx,int cy);
	class JUBASE_API GdiSelector : public _class{
	protected:
		HGDIOBJ _old;
		HDC		_dc;
	public:
	...
	JUBASE_API SIZE CalcTextSize(HDC hdc,String& text,HFONT font = 0);
	class JUBASE_API BitmapList : public _class{
	protected:
		HDC _dc;
