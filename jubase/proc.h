namespace ju{
	...
	class JUBASE_API _delegate : public _class
	{
	public:
	...
	//	]**/
	class JUBASE_API APIHook : public _class{
	protected:
		uchar* _orgFunc;
	public:
		APIHook();
...
#define GetThis(T,var) T* var;__asm mov var,edx
	...
	JUBASE_API void* GetNullProc(uint n);
	class JUBASE_API NullType{private:NullType(){}};
	template<typename T> struct JUBASETL_API is_reference{
		enum{value = false};
	};
	template<typename T> struct JUBASETL_API is_reference<T&>{
		enum{value = true};
	};
