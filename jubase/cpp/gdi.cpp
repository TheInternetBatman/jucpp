...
namespace ju{
	...
	bool Bitmap::PaintColor(int color,int x /* = 0 */,int y /* = 0 */,int cx /* = 0 */,int cy /* = 0 */){
		...
		if(cx<=0) cx = _Size.x;
		if(cy<=0) cy = _Size.y;
		if(x+cx>_Size.x) return false;
		if(y+cy>_Size.y) return false;
		HDC hdc = ::CreateCompatibleDC(0);
		HGDIOBJ hg = ::SelectObject(hdc,_Handle);
		Brush brush;
		brush.CreateDCBrush();
		brush.SetDCColor(hdc,color);
		::SelectObject(hdc,brush);
		BOOL r = PatBlt(hdc,x,y,cx,cy,PATCOPY);
		::SelectObject(hdc,hg);
		::DeleteDC(hdc);
		return r!=0;
	}
...
exit:
	...
	bool Bitmap::Draw(HBITMAP bmp,int x,int y,int cx,int cy,int sx,int sy,int scx,int scy,DWORD raster){
		if(!_Handle||!bmp) return 0;
		HDC bdc = ::CreateCompatibleDC(NULL);
		if(!bdc) return 0;
		HGDIOBJ old = ::SelectObject(bdc,bmp);
		bool r = Draw(bdc,x,y,cx,cy,sx,sy,scx,scy,raster);
		::SelectObject(bdc,old);
		::DeleteDC(bdc);
		return r;
	}
	...
	bool Bitmap::DrawAlpha(HBITMAP bmp,BYTE alpha,bool alphaChannel,int x,int y,int cx,int cy,int sx,int sy,int scx,int scy){
		if(!_Handle||bmp) return 0;
		HDC bdc = ::CreateCompatibleDC(NULL);
		if(!bdc) return 0;
		HGDIOBJ old = ::SelectObject(bdc,bmp);
		bool r = DrawAlpha(bdc,alpha,alphaChannel,x,y,cx,cy,sx,sy,scx,scy);
		::SelectObject(bdc,old);
		::DeleteDC(bdc);
		return r;
	}
