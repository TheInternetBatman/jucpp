...
namespace ju{
	...
	uint WcsFindNoCase(const wchar_t* wcs,const wchar_t* sub,uint maxlen){
	...
	uint WcsFind(const wchar_t* wcs,const wchar_t* sub,uint maxlen){
		if((wcs&&sub)==0) return -1;
		wchar_t* s = (wchar_t*)wcs,*s2 = (wchar_t*)sub;
		while(*s&&maxlen){
			s2 = (wchar_t*)sub;
	...
	LocalString::LocalString(LPCWSTR ws,uint cubage){
		...
		if(ws){
			len = WcsLength(ws);
			if(cubage<len) cubage = len;
		}else len = 0;
		if(cubage>LS_CUBAGE){
			_Handle = (wchar_t*)SYSAlloc(cubage*2+2);
			if(_Handle==0){
				_ASSERT(0);
				_Cubage = 0;
				return;
	...
	LocalString::LocalString(LocalString& ls){
		...
		if(len){
			memcpy(_Handle,ls._Handle,len*2+2);
			_Length = len;
		}else{
			_Handle[0] = 0;
			_Length = 0;
	...
	LocalString::LocalString(String& str){
		...
		if(len){
			memcpy(_Handle,str._Handle,len*2+2);
			_Length = len;
		}else{
			_Handle[0] = 0;
			_Length = 0;
	...
	LocalString::LocalString(LPCSTR mbs){
		...
		uint buflen = ::MultiByteToWideChar(CP_THREAD_ACP,0,mbs,len,0,0);
		if(buflen==0) return;
		if(buflen>_Cubage) SetCubage(buflen,1);
		_Length = ::MultiByteToWideChar(CP_THREAD_ACP,0,mbs,len,_Handle,buflen);
		_Handle[_Length] = 0;
	}
	bool LocalString::SetCubage(uint capacity,bool dbSpace){
		if(capacity<=LS_CUBAGE) return 1;
		capacity++;
		if(capacity<=_Length) return 1;
		int size;
		if(dbSpace){
			size = 8;
		...
		void* handle;
		if(_Handle==_buf){
			handle = SYSRealloc(0,size);
			if(handle==0) return 0;
			memcpy(handle,_Handle,_Length*2+2);
		}else{
			handle = SYSRealloc(_Handle,size);
			if(handle==0) return 0;
		}
		_Cubage = size/2 - 1;
	...
	void LocalString::operator = (LocalString& ls){
		uint len = ls._Length;
		if(len>_Cubage){
			if(!SetCubage(len)){
				_ASSERT(0);
	...
	void LocalString::operator = (String& str){
		uint len = str.Length();
		if(len>_Cubage){
			if(!SetCubage(len)){
				_ASSERT(0);
		...
		memcpy(_Handle,str.Handle(),len*2+2);
		_Length = len;
	}
	void LocalString::operator = (LPCWSTR wstr){
		...
		memcpy(_Handle,wstr,len*2+2);
		_Length = len;
	}
	void LocalString::operator = (LPCSTR mbs){
		uint len = MbsByteLen(mbs);
		if(len==0){
			_Handle[0] = 0;
			_Length = 0;
	...
	void LocalString::operator += (LocalString& ls){
		...
		if((ls.Length()+_Length)>_Cubage)
			SetCubage(ls.Length()+_Length,1);
		RtlCopyMemory(_Handle+_Length,ls.Handle(),ls.Length()*2);
		_Length += ls.Length();
		_Handle[_Length] = 0;
	}
	void LocalString::operator += (String& ls){
		if(ls.Length()==0) return;
		if((ls.Length()+_Length)>_Cubage)
			SetCubage(ls.Length()+_Length,1);
		RtlCopyMemory(_Handle+_Length,ls.Handle(),ls.Length()*2);
	...
	bool LocalString::operator == (LPCWSTR str){
		if(str==_Handle) return 1;
		if(!str) str = L"";
		for(uint i=0;i<_Length;i++){
			if(str[i]!=_Handle[i]) return 0;
		}
		return str[_Length]==0;
	}
	bool LocalString::operator != (LPCWSTR str){
		if(str==_Handle) return 0;
		if(!str) str = L"";
		for(uint i=0;i<_Length;i++){
			if(str[i]!=_Handle[i]) return 1;
		}
		return str[_Length]!=0;
	}
	//String
	...
	uint String::Find(wchar_t ch,uint start,uint maxlen){
		...
		if(maxlen>_Length-start) maxlen = _Length;
		for(uint i=start;i<maxlen;i++){
			if(_Handle[i]==ch)
				return i;
		}
		return -1;
	};
	uint String::Find(LPCWSTR wstr,uint start,uint maxlen){
		if(!_Handle||start>=_Length) return -1;
		uint p = WcsFind(_Handle+start,wstr,maxlen);
		if(p==-1) return -1;
		return p+start;
	}
	uint String::FindNoCase(wchar_t ch,uint start,uint maxlen){
		if(!_Handle||start>=_Length) return -1;
		if(maxlen>_Length-start) maxlen = _Length;
	...
	uint String::FindNoCase(LPCWSTR wstr,uint start,uint maxlen){
		if(!_Handle||start>=_Length) return -1;
		uint p = WcsFindNoCase(_Handle+start,wstr,maxlen);
		if(p==-1) return -1;
		return p+start; 
	}
	...
	void String::operator += (LPCSTR mbs){
		String str = mbs;
		*this += str;
	}
