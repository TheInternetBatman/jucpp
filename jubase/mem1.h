...
namespace ju{
	...
	struct JUBASE_API VirtualAddress : public HandleType<void*>{
	public:
		VirtualAddress();
	...
	class JUBASE_API IMemory : public _class{
	protected:
		class JUBASE_API CS : public _class{
		protected:
		...
		//此类只在函数中作为局部变量使用。
		class JUBASE_API LCS : public _class{
		protected:
			CS* _handle;
		public:
		...
		void*	_handle;
		uint	_length,_count,_total_used,_total_alloced;//每分配128个内存，重新从头查找可用空间
		char*	_curLarge,*_curSmall;//大内存和小内存的分配位置。
		CS		_cs;

		char* _findLargeSpace(uint len);
		char* _findSmallSpace(uint len);
	...
	public:
		IMemory();
		//对象是否为空.
		inline bool IsNull(){return _curLarge==NULL;}
		//分配内存
		void* Alloc(uint len);
	...
	class JUBASE_API ExecMemory : public IMemory{
	protected:
		HANDLE	_map;
	public:
