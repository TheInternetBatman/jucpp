#pragma once
namespace ju{
	JUBASE_API bool GetImageEncoderClsid(const wchar_t* type,CLSID* pClsid);
	...
	//Gdibmp是另一种图像资源,目前唯一的用途就是加载和保存不同类型的图像文件.用Gdibmp资源绘图效率是非常低的.
	class JUBASE_API Gdibmp : public _class{
	protected:
		void* _image;
	public:
		Gdibmp();
		...
		//bool Thumbnail(int cx,int cy,Gdibmp* srImg = 0);
		//复制
		void operator = (Gdibmp& img);
		//关闭，资源被清理。
		void Close();
