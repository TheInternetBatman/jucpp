...
namespace ju{
	...
	JUBASE_API bool CreateFolder(LPCWSTR path,bool file = 0);
	//删除一个文件或空文件夹。
	JUBASE_API bool RemoveFile(LPCWSTR file);
	//转换为全路径，name 返回文件名保护后缀，name 是 path 的某个位置。
	JUBASE_API bool FPToFullPath(String& path,LPCWSTR* name = 0);
	//比较两个字串是否表示相同的路径，但是不验证字串是否合法的路径名称。
	JUBASE_API bool FPIsSame(LPCWSTR path1,LPCWSTR path2);
	//取得字串的路径，末尾没有斜杠或反斜杠。如果字串本身是一个路径，则取得上一级目录。如果没有上一级目录，则字串不变化。
	JUBASE_API void FPToParent(String& ps);
	//把斜杠转换为反斜杠。
	JUBASE_API void FPToBackSlash(String& ps);
	//把反斜杠转化为斜杠。
	JUBASE_API void FPToSlash(String& ps);
	//取得后缀名，不包含字符"."。
	JUBASE_API void FPToExt(String& ps);
	//取得文件的标题（不包含后缀名的文件名），返回规则是先搜索末尾的“.”，再搜索斜杠或反斜杠，把之间的字串拷贝都title中。
	JUBASE_API void FPToTitle(String& ps);
	//取得文件名(文件标题和后缀)。
	JUBASE_API void FPToFileName(String& ps);
	//去掉末尾的斜杠或反斜杠。
	JUBASE_API void FPDelTailSlash(String& ps);
	//保持末尾有一个斜杠或反斜杠，表示字串是一个目录。
	JUBASE_API void FPSetTailSlash(String& ps);
	//链接两个字串，中间保持一个“.”号。
...
#define FS_MAX						10
	//文件系统提供文件的基本操作。
	class JUBASE_API FileSystem : public _class{
	protected:
		String _name;
	public:
		void* UserData;
	...
	//获取错误代码描述
	LPCWSTR JUBASE_API FSGetErrorMsg(uint fs_error);
	//文件夹操作使用的结构
	...
	class JUBASE_API  DSParam : public _class{
	public:
		void* UserData;
	...
	//文件夹操作
	class JUBASE_API DirectorySystem : public _class{
	protected:
	...
	//class FileWatch
	class JUBASE_API FileWatch : public _class
	{
	...
	class JUBASE_API File : public HandleType<HANDLE>
	{
	public:
		//构造
		File();
	...
	//class FileStream
	class JUBASE_API FileStream : public File
	{
	public:
...
//FILE_MAP_COPY		//内存是可读写的，但是写入数据不刷新到文件，实际上文件和映像是以只读方式打开的。
	class JUBASE_API FileMap : public HandleType<HANDLE>{
	protected:
	public:
		FileMap();
		~FileMap();
	...
	class JUBASE_API MapView : public HandleType<void*>{
	protected:
		uint _length;
	public:
		MapView();
