...
namespace ju{
	...
	JUBASE_API HICON GetFileIcon(LPCWSTR file,int index = 0);
	//设置一个文件的图标,iconfile可以是一个图标文件或可执行文件,iconindex是文件中图标的索引,toolinfo是提示信息.
	//这个函数是平台相关的,适用于XP,不保证适用于其它版本的操作系统.
	JUBASE_API bool SetFolderIcon(LPCWSTR folder,LPCWSTR iconfile,LPCWSTR iconindex = 0,LPCWSTR toolinfo = 0);
	//设置文件属性，add:此属性将被添加，del：此属性将被移除。
	JUBASE_API bool SetFileAttr(LPCWSTR file,DWORD add,DWORD del);
	//直接设置文件属性。
	JUBASE_API inline bool SetFileAttr(LPCWSTR file,DWORD attr){return 0!=SetFileAttributes(file,attr);}

	//搜索文件使用的数据结构
	typedef struct JUBASE_API FileSearchData : public _struct{
	...
	class JUBASE_API FolderWatch : public _class{
	public:
		enum WatchType{
	...
	class JUBASE_API MultiFolderWatch : public _class{
	protected:
