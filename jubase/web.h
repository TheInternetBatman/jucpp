...
namespace ju{
	...
	//把UTF8编码的字串还原。
	JUBASE_API void DecodeURI(String& str,DWORD codePage = CP_UTF8);
	//IP是网络地址的结构in_addr。
	typedef struct JUBASE_API IP : public _struct
	{
		union S_UN{
	...
	//Host表示一个主机结构，
	class JUBASE_API  Host : public _class
	{
		hostent* _Hostent;
	...
	//封装 Win Sock 函数
	class JUBASE_API  Socket : public HandleType<SOCKET>
	{
	public:
		Socket();
		~Socket();
	...
	class JUBASE_API  TCPServer : public _class
	{
	protected:
		Socket	_Handle;
		Thread _thread;
	...
	/*class JUBASE_API  TCPTransfer : public _class{
	protected:
		Socket				_handle;
	...
	class JUBASE_API  UDPSender : public _class
	{
	protected:
		Socket _Handle;
	public:
	...
	//UDPReceiver的析构函数会等到线程接收线程退出
	class JUBASE_API  UDPReceiver : public _class{
	protected:
		Socket _Handle;
		Thread _thread;
