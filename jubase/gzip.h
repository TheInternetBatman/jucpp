#pragma once
namespace ju{
	class JUBASE_API Ungzip : public _class{
	protected:
		void* strm;//z_stream 
	public:
		Ungzip();
	...
	class JUBASE_API Gzip : public _class{
	protected:
		void* strm;//z_stream 
	public:
		Gzip();
	...
	//返回压缩数据可能需要的最大缓冲区长度
	JUBASE_API uint GetMaxCompressLen(uint len);
	//压缩数据，返回压缩数据的长度：data：要压缩的数据源，length：源数据的长度，zData：压缩数据存放的缓冲区，bufLen：缓冲区的长度。
	JUBASE_API uint Compress(const char* data,uint length,char* zData,uint bufLen);
	//解压数据，返回解压数据的长度：zData：要解压缩的源数据，length：源数据的长度，data：解压数据的存放缓冲区，bufLen：缓冲区的大小。
	JUBASE_API uint DeCompress(const char* zData,uint length,char* data,uint bufLen);
	//压缩数据，返回压缩数据的长度：data：要压缩的数据源，zData：存放压缩数据的缓冲，不必设置大小，函数会自动调整。
	JUBASE_API uint Compress(const char* data,uint len,Memory<char>* zData);
	//解压数据，返回解压数据的长度：zData：要解压的数据源，data：存放解压数据的缓冲区，不必设置大小，函数会自动调整。
	JUBASE_API uint DeCompress(const char* zData,uint len,Memory<char>* data);
}