...
namespace ju{
	...
	JUBASE_API bool CreateXPStyleManifest();
	//关闭计算机,
	enum PowerMode
	{
		Power_Logoff = 0,//注销
	...
	JUBASE_API bool SetPower(PowerMode mode,bool force = 0);
	//如果调用线程阻塞，Timer将无法呼叫设定的回调函数。
	class JUBASE_API  Timer : public HandleType<UINT_PTR>
	{
	protected:
	...
	class JUBASE_API  Screen : public _class
	{
	public:
	...
	JUBASE_API bool SetProgramAutoRun(bool autorun,LPCWSTR keyName = 0,LPCWSTR file = 0,LPCWSTR arg = 0,bool alluser = 1);
	JUBASE_API bool GetProgramAutoRun(LPCWSTR keyName = 0,LPCWSTR file = 0);
	//这个类的方法全部为静态函数
	class JUBASE_API App{
	public:
